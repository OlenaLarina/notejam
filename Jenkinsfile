pipeline {
    ...
    steps {
        script {
            ...
            // Спробуємо авторизуватися через внутрішній сервіс метаданих Google
            // Це змушує gcloud "передумати" і взяти свіжі права
            sh "gcloud config set auth/impersonate_service_account 1077086262358-compute@developer.gserviceaccount.com || true"
